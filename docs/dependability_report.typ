...
#link("https://github.com/evanw/esbuild/security/advisories/GHSA-g7r4-m6w7-qqqr")[*GHSA-g7r4-m6w7-qqqr*]
riguarda una vulnerabilità di *path traversal* (CWE-22) nel
development server di esbuild: sfruttando separatori di percorso a
backslash (`\`), un attaccante può aggirare i controlli di
contenimento della directory `servedir` e leggere *file arbitrari*
dal filesystem. La causa risiede nell'uso di `path.Clean()`, che
normalizza solo i separatori forward-slash (`/`), invece di una
funzione di normalizzazione consapevole di Windows. La vulnerabilità
colpisce le versioni `>= 0.27.3` e `< 0.28.1` ed è corretta a
partire dalla versione *0.28.1*.
