...
<template>
  <ConfirmDialog
    ...
    message="Eliminare questa recensione? L'operazione è irreversibile."
  ...
  <div v-else-if="attivita" class="py-8">
    <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
      <div class="space-y-6 lg:col-span-1">
        ...
        <div class="h-64 overflow-hidden rounded-xl border border-gray-200 shadow-sm" id="activity-map" />
      </div>
      ...
      <div class="space-y-6 lg:col-span-2">
        ...
        <div v-if="isAlloggio" class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div v-else class="grid gap-4 sm:grid-cols-2">
            <div v-for="c in camere" :key="c.id" class="rounded-lg border border-gray-100 bg-gray-50 p-4">
              ...
              <p class="text-sm text-gray-500">Disponibilità: {{ c.disponibilita }}</p>
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div v-if="showReviewForm && auth.isVisitatore" class="mb-6 rounded-lg border border-gray-100 bg-gray-50 p-4">
            ...
            <input type="file" accept="image/*" @change="onReviewFile" class="mb-3 block text-sm text-gray-500 file:mr-3 file:rounded file:border-0 file:bg-green-50 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-green-700 hover:file:bg-green-100">
        ...
        <div v-if="auth.isLoggedIn && !auth.isAdmin" class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
          ...
          <div v-if="showSegnalazioneForm" class="rounded-lg border border-gray-100 bg-gray-50 p-4">
            ...
            <input type="file" accept="image/*" @change="onSegnalazioneFile" class="mb-3 block text-sm text-gray-500 file:mr-3 file:rounded file:border-0 file:bg-green-50 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-green-700 hover:file:bg-green-100">
